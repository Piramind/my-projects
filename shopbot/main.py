...
def send_text(message):
	...
	if message.text == "🎁 Купить":
		...
		if selectQiwi.status_code == 200:
			...
			try:
				bot.send_message(message.from_user.id, "QIWI кошелёк не работает!\nЗамените его")
			except requests.exceptions.ConnectionError:
				bot.send_message(message.from_user.id, "QIWI кошелёк не работает!\nЗамените его")
...
#Покупка товаров
		...
		if IdItems != 0: 
					...
					try:
						...
						if len(admin_id) > 1:
							for a in range(len(admin_id)):
								try:
									bot.send_message(admin_id[a], "Оплата не работает, укажите новые киви данные")
								except requests.exceptions.ConnectionError:
									bot.send_message(admin_id[a], "Оплата не работает, укажите новые киви данные")
						else:
							try:
								bot.send_message(admin_id[0], "Оплата не работает, укажите новые киви данные")
							except requests.exceptions.ConnectionError:
								bot.send_message(admin_id[0], "Оплата не работает, укажите новые киви данные")
					except requests.exceptions.ConnectionError:
						bot.send_message(call.message.chat.id, "❌ Извиняемся за доставленные неудобства,\nоплата временно не работает.", parse_mode = "MARKDOWN")
						if len(admin_id) > 1:
							for a in range(len(admin_id)):
								try:
									bot.send_message(admin_id[a], "Оплата не работает, срочно укажите новые киви данные")
								except requests.exceptions.ConnectionError:
									bot.send_message(admin_id[a], "Оплата не работает, срочно укажите новые киви данные")
						else:
							try:
			...
			try:
				bot.send_message(call.message.chat.id, "Данный товар не был найден.", parse_mode = "MARKDOWN", reply_markup = admin_keyboard)
			except requests.exceptions.ConnectionError:
				bot.send_message(call.message.chat.id, "Данный товар не был найден.", parse_mode = "MARKDOWN", reply_markup = admin_keyboard)
		...
		if allCheck:
			...
			if yesOrNo:
				...
				try:
					bot.send_message(call.from_user.id, "Ваша покупка не была найдена или была уже выдана.")
				except requests.exceptions.ConnectionError:
					bot.send_message(call.from_user.id, "Ваша покупка не была найдена или была уже выдана.")
		else:
			try:
				bot.send_message(call.from_user.id, "Пополнение не было найдено.\nПопробуйте чуть позже.")
			except requests.exceptions.ConnectionError:
				bot.send_message(call.from_user.id, "Пополнение не было найдено.\nПопробуйте чуть позже.")
